...
uint8_t checkInterruptReason(uint8_t ir) {
...
void isr() {
  // Check if POWOK (Power OK) interrupt occurred
  ...
  // If we get here, POWOK is still OK, clear interrupt flags
  resetInterruptReason();
  resetStatus();
}
...
void main_loop() {
  globals_init();
  pico_hal::init();
  gpio_set_irq_enabled_with_callback(pins::SW_POWER, GPIO_IRQ_EDGE_FALL, true,
                                     &gpio_callback);
  gpio_set_irq_enabled_with_callback(pins::SW_EJECT, GPIO_IRQ_EDGE_FALL, true,
                                     &gpio_callback);
  gpio_set_irq_enabled_with_callback(pins::DVD_EJECT, GPIO_IRQ_EDGE_FALL, true,
                                     &gpio_callback);
  gpio_set_irq_enabled_with_callback(pins::POWER_OK, GPIO_IRQ_EDGE_FALL | GPIO_IRQ_EDGE_RISE, true,
                                     &gpio_callback);
  pico_hal::set_fan_off();
  ...
  do {
    ...
    if ((power_standby_state & 1) != 0) {
      ...
      challenge.status_byte2 =
          challenge.status_byte0 + 22;  // TODO Feed number from timer
...
uint8_t update_power_standby() {
  ...
  switch (state.standby_power) {
    ...
    case power_standby_state::idle:
      ...
      pico_hal::timer1_wait();
      state.standby_power =
          power_standby_state::read_av;  // Loop back to AV reading
      return 0;
      break;
    ...
    case power_standby_state::turn_on_power_alternative:
      ...
      state.standby_power =
          power_standby_state::powered_up_wait_power_ok_alternative;
      break;
    ...
    case power_standby_state::powered_up:
      ...
      pico_hal::timer1_init(500);
      pico_hal::setupI2C();
      gpio_set_irq_enabled_with_callback(pins::POWER_OK, GPIO_IRQ_EDGE_FALL,
                                         true, &gpio_callback);
      return 1;
      break;
...
void update_fan_temp() {
  ...
  switch (state.fan_control) {
    ...
    case fan_control_state::decision_state:
      ...
      } else if (sensors.CPU_temperature > 74 ||
                 sensors.CPU_temp_predicted > 86) {
        /* CPU is warming up */
    ...
    case fan_control_state::state11:
      ...
      if ((flags.bitfield_DATA_74 & 0x01) == 0) {
        ...
        cpu_temperature_history[temperature_history_counter] =
            sensors.CPU_temperature;
        board_temperature_history[temperature_history_counter] =
            sensors.board_temperature;
...
uint8_t update_SMI_and_power() {
   ...
   * Returns 1 when system is powered off
   * State transitions: 0 -> (1,2,10); 1 -> (2,7); 2 -> (3,4,11); etc. */
  ...
  switch (state.power) {
    ...
    case 15:
      ...
      if ((sensors.tray_status == 0x00) || (sensors.tray_status == 0x40) ||
          (sensors.tray_status == 0x60)) {
        state.power = 7;
      } else if (--timers.power_timeout == 0) {
        state.power = 7;
      }
  ...
  switch (state.boot_challenge) {
    ...
    case boot_challenge_state::wait_for_ram_test_result:
      ...
      if (flags.bitfield_DATA_71 & 0x10) {
        ...
        state.boot_challenge =
            boot_challenge_state::challenge_failed_or_no_ram_test_result;
      }
      break;
    ...
    case boot_challenge_state::check_response:
      if (flags.bitfield_DATA_73 & 0x10) {
        ...
        if (challenge.response0 == challenge.expected0 &&
            challenge.response1 == challenge.expected1) {
...
void update_audio_clamp() {
  ...
  switch (state.audio_clamp) {
    ...
    case audio_state::tick_timer:
      ...
      if (!checkBitNo(flags.bitfield_DATA_71, 6) &&
          !checkBitNo(flags.bitfield_DATA_6F, 0) &&
          !checkBitNo(flags.bitfield_DATA_6F, 7)) {
        if (!checkBitNo(flags.bitfield_DATA_6F, 7)) {
    ...
    case audio_state::unclamped:
      ...
      if (checkStatusBit(prepare_for_shutdown) &&
          !checkBitNo(flags.bitfield_DATA_6F, 0) &&
          !checkBitNo(flags.bitfield_DATA_71, 6)) {
        return;
      }
      state.audio_clamp = audio_state::clamped;
      break;
  ...
  switch (state.pll_reset) {
    ...
    case pll_sysreset_state::state8:
      ...
      if (!pico_hal::get_power_ok()) {
        ...
        /* System reset requested */
        state.pll_reset =
            pll_sysreset_state::warm_reset_1;  // Go to warm reset path
      }
      break;
...
void read_temperatures() {
  ...
  // CPU
  if (pico_hal::read_byte(I2C_TEMP_SENSOR_ADDRESS, 0x01,
                          &(sensors.CPU_temperature))) {
    sensors.CPU_temperature_previous = sensors.CPU_temperature;
  }

  // Board, near MCPX for 1.0-1.4
  pico_hal::read_byte(I2C_TEMP_SENSOR_ADDRESS, 0x02,
                      &(sensors.board_temperature));
}
...
void boot_challenge_compute() {
  ...
  do {
    challenge.response0 =
        (challenge.input_byte0 << 2 ^ challenge.input_byte1 + 0x39 ^
         challenge.input_byte2 >> 2 ^ challenge.input_byte3 + 99U ^
         challenge.response1) +
        challenge.response0;
    ...
    challenge.response1 =
        (challenge.input_byte0 + 0xbU ^ challenge.input_byte1 >> 2 ^
         challenge.input_byte2 + 0x1b ^ challenge.response0) +
        challenge.response1;
